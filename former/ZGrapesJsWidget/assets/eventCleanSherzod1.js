...
function loadEditor() {
    ...
    $('.color-pick').on('click', function () {
        ...
        pickr.on('change', (...args) => {
            ...
            pickr.hide();
            let color = args[0].toHEXA()
            let savergbColor = `#${color[0]}${color[1]}${color[2]}`;
            let lStorage = localStorage.setItem('value', JSON.stringify(savergbColor));
            console.log(lStorage)
            $('.gjs-two-color').css({'background-color': localStorage.getItem('value'), 'color': lightOrDark(rgbColor)});
            $('.gjs-pn-views-container').css({'background-color': localStorage.getItem('value'), 'color': lightOrDark(rgbColor)});
            $('.gjs-pn-views').css({'background-color': localStorage.getItem('value'), 'color': lightOrDark(rgbColor)});
            $('.WhiteClass').css({'color': lightOrDark(rgbColor)});
...
function govnos() {
    ...
    var currentColor = localStorage.getItem('value');
    console.log(currentColor);
    var colorr = String(currentColor);
    console.log(colorr)

            $('.gjs-two-color').css({
                'color': lightOrDark(currentColor),
                'background-color': colorr
            });
            $('.gjs-pn-views-container').css({'background-color': currentColor, 'color': lightOrDark(currentColor)});
            $('.gjs-pn-views').css({'background-color': currentColor, 'color': lightOrDark(currentColor)});
            $('.WhiteClass').css({'color': lightOrDark(currentColor)});
