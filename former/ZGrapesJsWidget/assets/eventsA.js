...
function componentSelected(model) {
    ...
    $(divs).addClass('btn btn-outline-dark classes');
    $(divs).text('test');
    ...
    setTimeout(function () {
        ...
        let divses = $(parentDiv).find('.classes');
        var i = 0;
        if ($(parentDiv).find('.classes').length > 0) {


            for (i; divses.length > i; i++){
                $(divses[i]).remove();
                $(selectedEl).append(divs);
            }
