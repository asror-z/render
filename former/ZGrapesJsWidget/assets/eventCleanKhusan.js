...
function componentSelected(model) {
    ...
    settingsBtn.set('active', 0);
    let selected = editor.getSelected();
    let selectedEl = $(selected.getEl());
    let parentDiv = selectedEl;
    let id = 'opt-'+Math.random();
    let optionTag = document.createElement('div');
    $(optionTag).attr('id',id);
    $(optionTag).addClass('optionBtns');
    $(selectedEl).append(optionTag);


    $(selectedEl).click(function () {
        let positionX = event.pageX;
        let positionY = event.pageY;
        let elButtons = '<a class="btn-floating btn-sm btn-secondary position-absolute" style="top:' + positionY + '; left: ' + positionX + '; z-index: 1200"><i class="fas fa-star"></i></a>'
        let clickArr = [].push(elButtons);
            $(selectedEl).append(elButtons);
    ...
    $(parentDiv).attr('blockId', blockId);
...
function loadEditor() {
    ...
    const logo = "<img src='/render/former/ZGrapesJsWidget/demo/Grapes/css/img/zetsoft.png' width='100px' height='30px' class='logo mt-1'>";

    const eComp = document.createElement('div');
    $(eComp).addClass('components');

    $('.gjs-editor').append(eComp);

    const eCompButtonsParent = document.createElement('div');
    $(eCompButtonsParent).addClass('compBtnParent');

    $(eComp).append(eCompButtonsParent);

    const eFirstBtn = document.createElement('div');
    $(eFirstBtn).addClass('eBlocks');

    $(eCompButtonsParent).append(eFirstBtn);

    const eParams = document.createElement('div');
    $(eParams).addClass('eParams');

    $(eCompButtonsParent).append(eParams);

    const eColors = document.createElement('div');
    $(eColors).addClass('eColors');

    $(eCompButtonsParent).append(eColors);

    const ePut = document.createElement('div');
    $(ePut).addClass('eWidgets');

    $(eCompButtonsParent).append(ePut);

    const eSave = document.createElement('div');
    $(eSave).addClass('eSave');

    $(eCompButtonsParent).append(eSave);

    const eDelete = document.createElement('div');
    $(eDelete).addClass('eDelete');

    $(eCompButtonsParent).append(eDelete);

    const paintIcon = document.createElement('i');
    $(paintIcon).addClass('fal fa-brush blockIcon');
    $(eFirstBtn).append(paintIcon);

    const paramsIcon = document.createElement('i');
    $(paramsIcon).addClass('fal fa-cog paramIcon');
    $(eParams).append(paramsIcon);

    const blocksIcon = document.createElement('i');
    $(blocksIcon).addClass('fal fa-th-large paramIcon');
    $(eColors).append(blocksIcon);

    const brushIcon = document.createElement('i');
    $(brushIcon).addClass('fal fa-location-arrow paramIcon');
    $(ePut).append(brushIcon);

    const saveIcon = document.createElement('i');
    $(saveIcon).addClass('fal fa-save paramIconSave');
    $(eSave).append(saveIcon);

    const deleteIcon = document.createElement('i');
    $(deleteIcon).addClass('fal fa-trash-alt paramIconSave');
    $(eDelete).append(deleteIcon);

    $('.gjs-pn-panels').append(logo);

    let bm = editor.BlockManager;
    let pn = editor.Panels;
    let bs = editor.BlockManager.getAll();
    let titles = bs.models;
    let parentBlockss = bm.getContainer();
    ...
    let assetMened = editor.AssetManager;
    let childAssets = assetMened.getContainer();


    $(eColors).on('click', function () {
        ...
        $(flexContent).addClass('d-flex flex-wrap w-100 flex_content search-grape');
        let blocContent = document.createElement('div');
        $(blocContent).addClass('w-100');
        ...
        flexContent.append(blocContent);
        blocContent.append(parentBlockss);
    ...
    $(ePut).on('click', function () {
        window.open('{href}', '_blank');
    });

    $(eFirstBtn).on('click', function () {
