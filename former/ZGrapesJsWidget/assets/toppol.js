...
function componentSelected(model) {
    ...
    $(addWidget).addClass('btn-floating btn-sm btn-primary styling')
  ...
  <iframe class="embed-responsive-item" id="contents" src='/core/kernel/grapes/templates.aspx' width="400px" height="600px"></iframe>           
</div>`,
        ...
        $(iframe).on('load', function () {
            ...
            $(btn).on('click', function () {
                ...
                $.ajax({
                    type: 'GET',
                    url: '/core/kernel/grape/getBlockAssets.aspx',
                    data: {
                        param: dataId
                    },
                    success: function (response) {
                ...
                $.ajax({
                    type: 'GET',
                    url: '/core/kernel/grape/getBlocks.aspx',
                    data: {
                        param: dataId
                    },
                    success: function (response) {
                        /* response = '<div class="zwidget parent-div">' + response + '</div>'
                        $(selectedEl).append(response)
                        Swal.close()*/
                        const domComponents = editor.DomComponents;
    ...
    $(addWidget).on('click', function () {
        ...
        Swal.fire({
            title: 'widgets',
            html: `<div class="embed-responsive w-100 h-100 embed-responsive-16by9">
  <iframe class="embed-responsive-item" id="contents" src='/core/kernel/grapes/widgets.aspx' width="400px" height="600px"></iframe>           
</div>`,
        ...
        $(iframe).on('load', function () {
            ...
            $(btn).on('click', function () {
                ...
                $.ajax({
                    type: 'GET',
                    url: '/core/kernel/grape/getAssets.aspx',
                    data: {
                        param: dataId
                    },
                    success: function (response) {
                ...
                $.ajax({
                    type: 'GET',
                    url: '/core/kernel/grape/getWidget.aspx',
                    data: {
                        param: dataId
                    },
                    success: function (response) {
...
function govnos() {
  ...
  */
    ...
    let videoElementsParent = document.createElement('div')
    $(videoElementsParent).addClass('w-100 pt-3 d-flex flex-column justify-content-center align-items-center')

    let firstVideo = document.createElement('a')
    $(firstVideo).css({
        'padding': '15px',
    ...
    $(firstVideo).text('Инструкция конструтора')

    let secondVideo = document.createElement('a')
    $(secondVideo).css({
        'padding': '15px',
        'border': '1px solid gray',
        'border-radius': '20px',
    ...
    $(secondVideo).text('Инструкция виджетов')

    $(videoElementsParent).append(firstVideo)
    $(videoElementsParent).append(secondVideo)

    let videosDiv = document.createElement('div')
    $(videosDiv).addClass('d-none vids')
    $(videosDiv).html(videoElementsParent)
    $(container).append(videosDiv)

    $(firstVideo).on('click', function () {
        Swal.fire({
            title: 'blocks',
            html: `<iframe width="100%" height="600px" src="https://www.youtube.com/embed/2YPCsOng5UA" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`,
