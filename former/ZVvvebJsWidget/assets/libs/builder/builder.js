...
Vvveb.Components = {
  ...
  render: function (type) {
    ...
    var storageWidget = localStorage.getItem('widgetName');
      ...
      if (property.init) {
        ...
        if (property.htmlAttr == 'style') {
          ...
          if (storageWidget) {
            ...
            if (property.htmlAttr == null) {
              property.htmlAttr = 'div';
            }

            $.ajax({
              url: '/core/grape/getWidget.aspx?param=' + storageWidget,
              method: 'GET',
              success: (response) => {
                value = response;
              },
...
Vvveb.Builder = {
  ...
  loadControlGroups: function () {
    ...
    componentsList.each(function () {
      ...
      for (group in Vvveb.ComponentsGroup) {
        ...
        componentsSubList = list.find('li[data-section="' + group + '"]  ol');
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response) {
          ...
          console.log(response);
          componentsList.append(response);
        }
  ...
  _initDragdrop: function () {
    ...
    setTimeout(function () {
      var widgets = $(document).find('.widgets');

      $.each(widgets, function (key, value) {
        $(value).on('mousedown touchstart', function (event) {

          $this = $(this);
          $('#component-clone').remove();

          if ($this.data('drag-type') == 'widget') {
            var widget = $this.data('widget');

            self.isDragging = true;

            localStorage.setItem('widgetName', widget);
          }
