...
//Comments (2)
// OllieJones@v1.10.1216:35, Thu 9th Jun 2016
// I needed to print a table containing some PNG images. To do this (Allan explained) means shutting off automatic HTML stripping from the print module.
//
// But, when I shut off HTML stripping entirely, I also got some hyperlinks printed. The solution that worked for me was to implement a body formatting function. The whole thing, including some other vanilla-flavored buttons, looks like this. It retains strong, sup (superscript), and img tags, and gets rid of the rest.
...
//DataTables initialisation: Use the HTML5 PDF button:
...
//DataTables initialisation: Insert an image into the document:
...
//In order to place a header or footer on the document, you must add them at the document level, NOT the document content level. Here's an example:
...
//DataTables initialization: Use the customize option to add a green background and thin border to the first row (e.g. column headers). This is a built in style. See the list above for other styles.
...
//F12Magic@v1.10.1216:32, Wed 20th Jul 2016
...
//https://datatables.net/reference/button/csvFlash
...
//Comments (1)
/*
zebraI@v1.10.1510:33, Tue 16th May 2017
Hi, I was looking for some help for customizing my CSV export. As I finally managed to do it, I'll post my solution here because I couldn't find much stuff:
*/
...
//https://datatables.net/reference/button/colvisRestore
...
//https://datatables.net/reference/button/colvisGroup
...
//Direct initialisation: Show the colvis button, but do not include the first column in the list of columns:
...
//https://datatables.net/reference/button/columnsVisibility
...
//https://datatables.net/reference/button/columnsToggle
...
//Direct initialisation: Show a button for every column in a collection (this is the same as the colvis button type):
...
//https://datatables.net/reference/button/columnVisibility
...
//Direct initialisation: A single button that will toggle the visibility of columns with an index greather than 4:
...
//Direct initialisation: Collection of custom buttons:
