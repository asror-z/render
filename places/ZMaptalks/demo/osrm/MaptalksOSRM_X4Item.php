...
if (!isset($item)) {
    $item = new MapItem();
    $item->finishLatLng = [69.24881988859192, 41.31838202555613];
}
...
<script>
    ...
    // point with altitude
    var latLongFinish = <?=json_encode($item->finishLatLng)?>;
        ...
        fetch(`https://router.project-osrm.org/route/v1/driving/${currentP};${latLongFinish}?overview=false&geometries=geojson&alternatives=true&steps=true`)
            ...
            .then(data => {
                ...
                for (var j = 0; j < coords.length; j++) {
                    ...
                    switch (true) {
                        case direction <= 22 && direction >= 338 :
                            azimuth = "north";
                            break;
                        case direction > 22 && direction < 68 :
                            azimuth = "northeast";
                            break;
                        case direction >= 68 && direction <= 112 :
                            azimuth = "east";
                            break;
                        case  direction > 112 && direction <= 157 :
                            azimuth = "southeast";
                            break;
