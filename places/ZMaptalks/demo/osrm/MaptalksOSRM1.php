...
<head>
    ...
    <!--
    //start|XakimjonErgashov|2020-10-01
    02.10.2020
    20 lines css = 16
    5 lines html = 5
    23 lines JS = 46
        ...
        fetch(`https://router.project-osrm.org/route/v1/driving/${currentP};${latLongFinish}?overview=false&geometries=geojson&alternatives=true&steps=true`)
            ...
            .then(data => {
                ...
                currentDiv.addEventListener('click', function (e) {
                    ...
                    //console.log(event.toElement.lastChild.innerText);
                    let clickedDiv = e.toElement.lastChild.innerText;
                    /*clickedDiv.push(e.toElement.lastChild.innerText)*/;

                    //parseFloat(clickedDiv);

                    //let myVar =  clickedDiv.replace(/^"(.+(?="$))"$/, '$1');
                    //clickedDiv.replace(/['"]+/g, '');

                    console.log(parseFloat(clickedDiv));
