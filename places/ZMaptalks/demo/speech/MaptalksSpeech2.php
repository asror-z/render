...
//start|XakimjonErgashov|2020-10-17
...
<div>
    <div class="speechDiv"><input type="text" class="speechInput" id="text-to-speech"
                                  placeholder="Enter text to speak..."/>
        <button class="speechButton" type="button">Speak</button>
    </div>
...
<script>
    ...
    var resolutions = [];
    var speechButton = document.querySelector('.speechButton');
    ...
    function success(data) {
        ...
        /*    console.log('...... ' + v);
          if(JSON.stringify(v) === JSON.stringify(lt)){
              console.log('equal' + v);
          }else{} */
        v = lt;
        //console.log('NotEqual' + v);
        ...
        fetch(`https://router.project-osrm.org/route/v1/driving/${currentP};${latLongFinish}?overview=false&geometries=geojson&alternatives=true&steps=true`)
            ...
            .then(data => {
                ...
                for (var j = 0; j < coords.length; j++) {
                    var steps = routes[0].legs[0].steps[j];
                    ...
                    for (var j = 0; j < addressCollection.length; j++) {
                        console.log(addressCollection[j])
                        addressCollection.push(adressName);
                    }
                   
                    const stepPosition = inRoute[0];

                    function directionToAzimuth(degree) {

                        switch (true) {
                            case degree <= 22 || degree >= 338:
                                azimuth = "north";
                                break;
                            case degree > 22 && degree < 68:
                                azimuth = "northeast";
                                break;
                            case degree >= 68 && degree <= 112:
                                azimuth = "east";
                                break;
                            case degree > 112 && degree <= 157:
                                azimuth = "southeast";
                                break;
                            case degree >= 158 && degree < 203:
                                azimuth = "south";
                                break;
                            case degree >= 203 && degree < 248:
                                azimuth = "southwest";
                                break;
                            case degree >= 248 && degree < 293:
                                azimuth = "west";
                                break;
                            case degree >= 293 && degree < 338:
                                azimuth = "northwest";
                                break;
                    ...
                    var azimuthName = directionToAzimuth(direction);
                    ...
                    if (modifier == undefined || modifier == null) {
                        ...
                        var updatedType = setLanguage(maneuver, azimuthName);
                        ...
                        var updatedManeuver = setLanguage(maneuver, azimuthName);
                ...
                var currentText = routes[0].legs[0].steps[0].name;
                var currentManeuver = routes[0].legs[0].steps[0];
                var firstDirection = routes[0].legs[0].steps[0].maneuver.bearing_after;
                var firstAzimuth = null
                var firstAzimuthName = directionToAzimuth(firstDirection);

                var firstManeuver = setLanguage(currentManeuver, firstAzimuthName);

                function startSpeech() {

                    if ('speechSynthesis' in window) {
                        speechButton.addEventListener('click', function () {
                            let msg = firstManeuver + currentText;
