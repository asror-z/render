...
class elFinderVolumeBox extends elFinderVolumeDriver
{
     ...
     * @var string
     */
    const FETCHFIELDS = 'type,id,name,created_at,modified_at,title,size,parent,permissions,file_version,shared_link';
    ...
    protected function getSharedWebContentLink($raw)
    {
        ...
        if ($raw->shared_link->url && ($fType == 'image' || $fType == 'video' || $fType == 'audio')) {
            if ($fExtension == 'jpg' && $fType == 'image') {
                ...
                $url = 'https://app.box.com/index_product.php?rm=preview_stream&amp&file_version_' . $raw->file_version->id . '/audio/mpeg:' . $raw->name . '&shared_name=' . basename($raw->shared_link->url);
