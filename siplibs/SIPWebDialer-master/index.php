...
// EVENTS
function Event(){
        SIP.on('registered', function (event){ 
        ...
        SIP.on('unregistered', function (event){  
        ...
        SIP.on('registrationFailed', function (event){  
        ...
        SIP.on('newMessage', function (event){  
		alert('Nouveau message recu'); 
	});

        SIP.on('connected', function (event){});

        SIP.on('disconnected', function (event){
		$('.inforegister').html("Server Unavailable").parent().removeClass('alert-info').addClass('alert-danger'); 
	});
	
	SIP.on('newRTCSession', function (event){
		console.log(e);
		if(rtcSession){
...
function sendMessage( text , uri){
        var eventHandlers = {
          'succeeded': function (event){ /* Your code here */ },
          'failed':    function (event){ /* Your code here */ }
        };
