...
const boxes = Array.from(contain.getElementsByClassName("box"));
...
const selectable = new Selectable({
    ...
    /*classes contain - the contain defined by the appendTo option
...
function init() {
    ...
    arr.push(m.add(selectable.config, 'toggle').name("Enabled").onChange((bool) => {
        selectable.clear();
        selectable.contain.classList.toggle(selectable.config.classes.multiple, bool);
    }));
    ...
    arr.push(d.add(Misc, "random").onChange(togglecontain));
    arr.push(d.add(Misc, "fixed").onChange(togglecontain));
    arr.push(d.add(Misc, "transformed").onChange(togglecontain));
    d.open();

    function togglecontain(bool) {
        const rand = this.property === "random" || this.property === "transformed";
        contain.classList.toggle("random", rand);
        selectable.setcontain(rand ? document.body : contain);
