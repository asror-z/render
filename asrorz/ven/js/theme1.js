...
jQuery(document).on('pjax:end', function () {
   /* if (Boolean(Cookies.get('clicked')) === false) {
        return Cookies.set('clicked', false, {path: '/'});
    } else {
        all();
        Cookies.set('clicked', false, {path: '/'})
    }*/
    $("#messagePjax").click(function () {
        configSetCookie("clicked", true)
        if (configGetCookie("clicked") == true) {
            console.log("TRUE")
            all()
        } else if (configGetCookie("clicked") == false) {
            console.log("FALSE")
            return false
        }
