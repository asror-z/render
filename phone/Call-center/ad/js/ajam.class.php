...
private function parseRaw($data)
	{
		...
		foreach ($rows as $str_line) {
			if ((preg_match('/Response:|Message:/i', $str_line))) {
				...
				if ((preg_match('/^\s+|END\sCOMMAND/i', $str_line))) {
					...
					if (preg_match('/: /', $str_line)) {
						 ...
						 *
						 * Eg.
						 * $arr_ret['Test'][0] = [Code][010];
						 */
						if ((!is_null($str_line)) && (count($arr_ret["Test"][$int_cont]) == 0) && ($str_key)) {
							$arr_ret["Test"][$int_cont] = array($str_key => $str_val);
						} else {
							if ($str_key) {
								/**
								 ...
								 */
								$arr_ret["Test"][$int_cont][$str_key] = $str_val;
							}
						}
					} else {
					/**
					 ...
					 */
						if ((!is_null($str_line)) && (count($arr_ret["Test"][$int_cont]) == 0)) {
							$arr_ret["Test"][$int_cont] = array(trim($str_line));
						} else {
							array_push($arr_ret["Test"][$int_cont], trim($str_line));
						}
					}
				}
			}
		}
		array_pop($arr_ret["Test"]);
		unset($int_cont);
		return $arr_ret;
	}
	private function parseRaw2($data) {
		  ...
		  if( preg_match('/Output:/i',$value) == true ) {
		   $pair = explode(':', $value);
		   $result[ $pair[0].($key - 2) ] = $pair[1];
         } else {
		   $pair = explode(':', $value);
	...
	private function parseRawTest($data) {
	  $data = explode("\r\n", $data);
/*  	   $result = array();
        foreach($data as $str_line) {
		     $str_line = preg_replace('/\\r\\n/i', ',', $str_line);
	         $str_line = explode(',', $str_line);
	         $str_line = array_filter($str_line);
               array_walk($str_line, function (&$value, $key) use (&$result){
		  if( preg_match('/Output:/i',$value) == true ) {
		   $pair = explode(':', $value);
		   $result[ $pair[0].($key - 2) ] = $pair[1];
         } else {
	...
	public function doCommand2($action_str, $params_arr = array())
	{
		if (!$this->checkCon()) {
			if ($this->_debug) {
				echo "Need login \n";
		...
		if (($action_str)) {
			$query = $this->buildQuery($action_str, $params_arr);
			$data = $this->getResponse("command", $query);
			$raw_result = $this->parseRaw2($data);
	 ...
	 * @return array
	 */
	public function doCli($action_str)
	{
			$this->doLogin($this->_authtype);
			$query = $this->_urlraw . "?action=Command&Command=" . $action_str;
			$data = $this->getResponse("command", $query);
			$raw_result = $this->parseRaw($data);
			return $this->setResult($raw_result);
	}
