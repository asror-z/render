...
class Webrtc extends \FreePBX_Helpers implements \BMO {
	...
	/**
	 * Supported Versions of asteriskk for this module
	 * @type {array}
	 */
	...
	*/
	...
	public function validVersion() {
		...
		if(isset($this->supported[$base])) {
			if(!version_compare($version, $this->supported[$base], "ge")) {
				return sprintf(_("Unsupported Version of asteriskk, You need at least %s you have %s"), $this->supported[$base], $version);
			}
		} else {
			return sprintf(_("Unsupported Version of asteriskk, You need at least %s you have %s"), $this->supported["11"], $version);
		}
		return true;
	}
	...
	public function getClientSettingsByUser($user) {
		...
		if($secure && !$this->freepbx->Config->get('HTTPTLSENABLE')) {
			return array("status" => false, "message" => _("HTTPS is not enabled for asteriskk"));
		}
	...
	public function createDevice($extension) {
		...
		//$settings['callerid']['value'] = $dev['title'] . "<".$extension.">";
		$defaultCert = $this->certman->getDefaultCertDetails();
		if(empty($defaultCert)) {
			return false;
		}
