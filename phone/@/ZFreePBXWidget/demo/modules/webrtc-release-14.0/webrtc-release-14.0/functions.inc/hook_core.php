...
function webrtc_configpageinit($pagename) {
	...
	$action = isset($_REQUEST[paramAction])?$_REQUEST[paramAction]:null;
	...
	if($status === true) {
		if(!empty($mcerts)) {
			if($mode == 'pjsip') {
				...
				_('Enable WebRTC Old ARI Phone'), sprintf(_('Enables WebRTC for this %s in the asteriskk Recording Interface (ARI). Note: ARI is depreciated in favor of UCP'),$mode), false));
				$certs = array();
