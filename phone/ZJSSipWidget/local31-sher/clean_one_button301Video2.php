...
if(!isset($item)){
    $item = new SIPMLItem();
    $item->impu = '@10.10.3.41:5060';
    $item->impi = '301';
    $item->password = 301;
    $item->websocket_proxy_url = 'wss://10.10.3.41:8089/ws';
    $item->keypad = true;
}
...
<script>
    ...
    function add_stream_video() {
        ...
        console.log(session.connection.getAudioTracks().length);
        session.connection.addEventListener('addstream', function (e) {
            console.log(remoteView);
            console.log(session.getRemoteStreams()[0].getAudioTracks().length);
