...
<script>
    ...
    function chekingSdp(session){
    console.log("chekingSdp");
    console.log("chekingSdp");
    console.log("chekingSdp");
    console.log("chekingSdp");
    console.log(session._request);
    console.log(session._request.sdp);
        let chekTypeVar = session._request.sdp.media; // достаю все медиа потоки
        //проверка на тип звонка (Видел или аудио)
        for(var i = 0 ; i < chekTypeVar.length ; i++ ){
            if(session._request.sdp.media[i].type == "video"){
                console.log(session);
