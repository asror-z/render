...
<!--<script src="https://cdn.statically.io/gh/versatica/JsSIP/master/dist/jssip.min.js"></script>-->
<script src="https://cdn.statically.io/gh/versatica/JsSIP/3.4.6/dist/jssip.min.js"></script>
<style>
...
<div class="container">
    <div class="row">
        <div class="number text-center border col-4 w-100">
            <div class="mb-2 mt-2 control_btn">
                ...
                <button id="videoBtn" class="btn btnVideoCall py-2">
                    <i class="fas fa-video fa-2x" style="color: lime"></i>
                </button>
        ...
        <div class="w-100 position-relative">
            ...
            <video class="position-absolute"
                   style="bottom: 10px; right: 10px; width: 25%; box-shadow: 0px 0px 17px 0px #f5f4f4;" autoplay
                   id="selfView" muted></video>
        </div>
        ...
        <div id="divCallCtrl" class="p-0 m-0">
            ...
            <div id='divCallOptions' class='call-options text-center'>
                ...
                <button class="btn px-3 py-2 " id="btnTransfer" onclick='sipTransfer();'><i class="fas fa-random"></i>
                </button>
                <button class="btn px-3 py-2 " id="btnLocalVid" onclick="localVidOn()"><i id="localVidIcon"
...
<script>
    ...
    if (user && pass) {
        ...
        var checkTypeVideo = false;

        phone.on('newRTCSession', (data) => {
            var newSession = data.session;

            if (data.originator == "remote") {
                checkTypeVideo = data.request.data.indexOf('m=video') !== -1;
                console.log(checkTypeVideo);
            }
            ...
            var completeSession = function () {
                ...
                txtRegStatus.innerHTML = 'Вызов завершен';
                $("#jspanel").hide();
                isVideo = false;
                stopTimer();
                setTimeout(function(){ txtRegStatus.innerHTML = 'Соединено'; timer.classList.add('d-none'); resetTimer();}, 1500);
                calling.pause();
                ringing.pause();
                session = null;
            };
            ...
            if (session.direction == 'outgoing') {
                ...
                txtRegStatus.innerHTML = 'Входящий вызов';
                ...
                session.on('sdp', function (e) {
                    checkVideo = e.sdp.indexOf('m=video') !== -1;
                    if (checkVideo) {
                        videoTrue();
                    }
                    timer.classList.remove('d-none');
        ...
        phone.on('connecting', (data) => {
            ...
            txtRegStatus.innerHTML = 'Соединяется';
        });
        phone.on('connected', (data) => {
            //let session = data.session;

            console.log('connected');
            txtRegStatus.innerHTML = 'Соединено';
        });
        phone.on('disconnected', (data) => {
            //let session = data.session;

            console.log('disconnected');
            txtRegStatus.innerHTML = 'Не соединено';
    ...
    videoCallBtn.addEventListener('click', function () {
        if (!isVideo) {
            if (txtPhoneNumber.value == "" || txtPhoneNumber.value == null){
