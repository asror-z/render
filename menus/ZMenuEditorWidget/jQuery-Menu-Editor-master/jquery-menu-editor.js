...
function MenuEditor(idSelector, options) {
    ...
    $(document).on('click', '.btnEdit', function(e) {
        ...
        var icon_val = $('#iconValue').val(iconPicker.find('i')[0].className);
        $('#myEditor_icon').val(icon_val);
        var icon_pick = iconPicker.find('i')[0].className;
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var itemObject = {text: "", icon: "", param: "", target: "_self", action: "",};
            var temp = $.extend({}, v);
            if (isParent){
    ...
    this.update = function() {
        ...
        var icon_val = $('#iconValue').val(iconPicker.find('i')[0].className);
        $('#myEditor_icon').val(icon_val);
        var icon_picker = iconPicker.find('i')[0].className;
        ...
        $($cEl.find('i')[0]).addClass(iconPicker.find('i')[0].className);
    ...
    this.add = function(){
        ...
        var textItem = $('<span>').addClass('txt').text($('#text_value').val());
        var iconItem = $('<i>').addClass(iconPicker.find('i')[0].className);
