...
if (!isset($item)) {
    ...
    $item->discount = -10;
    $item->catalogId = 19;
    $item->max_price = 50000;
    $item->min_price = 30000;
    $item->is_multi = false;
        ...
        <a href="<?= $item->url ?>" class="position-relative overflow-hidden zvertical-card"
           style='background-image: url("<?= $item->images[0] ?>")'>
            <div class="position-absolute" style="right: -10%; top: 5%">
                <? if (in_array(ProductItem::statuses['top_sell'], $item->status) && $item->is_multi) { ?>
                    <div class="ml-auto red-bg-color text-white rounded-left rounded-pill fe-08  py-1 pl-lg-3 pr-xl-5 pl-2 pr-2">
                        <?= Az::l('Топ продаж') ?>
                    </div>
                <? } ?>

                <? if ((in_array(ProductItem::statuses['sale'], $item->status) && in_array(ProductItem::statuses['new'], $item->status))&&$item->is_multi) { ?>
                ...
                <? } elseif (in_array(ProductItem::statuses['new'], $item->status) && $item->is_multi) { ?>
                    <div class="ml-auto green-bg-color text-white rounded-left rounded-pill fe-08  py-1 pl-lg-3 pr-xl-5 pl-2 pr-2">
                        <?= Az::l('Новинка') ?>
                    </div>
                <? } ?>
                ...
                <div class="w-100">
                    <div class="d-flex justify-content-center flex-wrap">
                        <div class="h6 fe-08 text-center text-success">
                            ...
                            if (!$item->is_multi)
                                echo number_format($item->new_price, 0, '.', ' ');
                            else {
                                echo number_format($item->min_price, 0, '.', ' ');;
                                if ($item->max_price !== null)
                                    echo ' - ' . number_format($item->max_price, 0, '.', ' ');
                                else
                                    echo '';
                            }
                            ?>
        ...
        <div class="mx-2 footer-main">
            <? if (!$item->is_multi) : ?>
                <button id="add-card-<?= $item->id ?>"
                        class="w-100 mx-0 add-card btn button-bg-color btn-sm text-white <?= $item->amount > 0 ? "d-none" : "" ?>"
                        onclick="addToCart($(this))"
                        data-catalog-id="<?= $item->catalogId ?>">
                    <?= Az::l('В корзину') ?>
                </button>

                <div id="input-<?= $item->id ?>" class="touch-main <?= $item->amount > 0 ? " " : "d-none" ?>">
...
<script>
    ...
    {
        $("#input-<?=$item->id?> input").val(<?= $item->amount ?>);
    }
</script>
